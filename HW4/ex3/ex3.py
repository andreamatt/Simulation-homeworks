...
import numpy as np
from numpy import mean, min, max, median, quantile, sqrt, log
...
expected_value = 0
figs[0].plot([0, max_time], [expected_value, expected_value], 'r-', linewidth=1)
for i in range(len(ts)):
	t = ts[i]
	m = means_per_t[i]
	s = std_per_t[i]
...
expected_value = (max_speed-min_speed)/log(max_speed/min_speed)
figs[1].plot([0, max_time], [expected_value, expected_value], 'r-', linewidth=1)
for i in range(len(ts)):
	t = ts[i]
	m = means_per_t[i]
	s = std_per_t[i]
...
expected_value = (max_speed-min_speed)/log(max_speed/min_speed)
figs[2].plot([0, max_time], [expected_value, expected_value], 'r-', linewidth=1)
for i in range(len(ts)):
	t = ts[i]
	m = means_per_t[i]
	s = std_per_t[i]
	...
	hist = hist[valid_indexes]
	bin_edges = bin_edges[valid_indexes]
	figs[2].plot(t - hist * t_interval, bin_edges + bin_w / 2, 'b-', linewidth=1, zorder=3)
	figs[2].plot([t, t], [min_h - bin_w / 2, max_h + bin_w / 2], 'b-', linewidth=0.5)
	figs[2].plot([t, t], CI, 'r-', linewidth=2)
	figs[2].plot([t], [m], 'ko', markersize=3)
