...
class Relay:
	...
	def __init__(self, data):
		...
		self.ShouldBeAwake = data['ShouldBeAwake']
		...
		self.finishedCTSs = []
		self.packetToSend = None if data['packetToSend'] == None else Packet(
			data['packetToSend'])
...
class Plot:
	...
	def Init(data):
		...
		Plot.relay_markers = {}
		Plot.show_Ids = False
		Plot.show_dutyCicle = True
		Plot.frames = data["Frames"]
		Plot.frame_index = 0

		# zoom and drag
		Plot.center = (Plot.sim_params.area_side/2,
					   Plot.sim_params.area_side/2)
		Plot.scale = 1
	...
	def KeyPress(event):
		if(event.key == "2"):
			Plot.increaseIndex() if Plot.show_dutyCicle else Plot.increaseIndex_recursive()
		elif(event.key == "1"):
			Plot.decreaseIndex() if Plot.show_dutyCicle else Plot.decreaseIndex_recursive()
		elif(event.key == "-"):
			Plot.scale += 0.1
		elif(event.key == "+"):
			...
			Plot.show_dutyCicle = not Plot.show_dutyCicle
	...
	@staticmethod
	def increaseIndex_recursive():
		if Plot.frame_index < len(Plot.frames)-1:
			Plot.frame_index += 1
			currentEvent = Plot.frames[Plot.frame_index]['currentEvent']['type']
			while currentEvent == 'AwakeEvent' or currentEvent == 'SleepEvent':
				Plot.frame_index += 1
				if Plot.frame_index == len(Plot.frames)-1:
					break
				currentEvent = Plot.frames[Plot.frame_index]['currentEvent']['type']
...
class Frame_plotter:
	...
	def plot_relay(self, relay):
		if relay.status=='Asleep' and relay.ShouldBeAwake:
			relay.status = 'Free'
		relay_pos = (relay.X, relay.Y)
		text_pos = (relay.X, relay.Y + 2)
		relay_marker = plt.Circle(relay_pos, radius=Plot.dot_radius,
								  facecolor=Plot.relay_colors[relay.status], edgecolor='black')
		Plot.ax1.add_patch(relay_marker)
		Plot.relay_markers[relay.id] = relay_marker

		if Plot.show_Ids:
			Plot.ax1.annotate(relay.id, xy=relay_pos, xytext=text_pos, color='black',
							  weight='medium', ha='center', va='bottom', bbox=dict(boxstyle="round", fc="w"))

		modality = Plot.relay_details[relay.id]

		if modality != info_modality.off.value:
			Plot.ax1.annotate(relay.details(modality), xy=relay_pos, xytext=text_pos, color='black',
							  weight='medium', ha='center', va='bottom', bbox=dict(boxstyle="round", fc="w"))
			if modality == info_modality.packet_info.value and relay.packetToSend != None:
				...
				Plot.ax1.arrow(relay.X, relay.Y, sink.X - relay.X, sink.Y - relay.Y,
							   length_includes_head=True, head_width=2, head_length=3, capstyle="butt", ls="-")
	...
	def plot_arrow(self, relay):
		if relay.status == "Transmitting" and relay.transmissionType == 'PKT':
			...
			Plot.ax1.arrow(relay.X, relay.Y, destination.X - relay.X, destination.Y - relay.Y,
						   length_includes_head=True, head_width=2, head_length=3, capstyle="butt", ls="--")
	...
	def plot_signal(self, relay, signal=None):
		...
		if relay.status == "Transmitting":
			...
			relay_range = plt.Circle(
				relay_pos, radius=relay.range, facecolor=color, alpha=alpha, edgecolor='black')
	...
	def plot_regions(self, relay):
		if relay.status == "Transmitting" and relay.transmissionType == "RTS":
			...
			for i in range(1, regions+1):
				a = sg.Point(sink.X, sink.Y).buffer(
					dist - i/regions * relay.range)
				diff = starting.difference(a)
				if region_index == regions-i:
					Plot.ax1.add_patch(descartes.PolygonPatch(
						diff, fc='#00ff00', ec='k', alpha=0.15))
				else:
					Plot.ax1.add_patch(descartes.PolygonPatch(
						diff, fc='#000000', ec='k', alpha=0.10))
				starting = starting.intersection(a)

	def plot(self):
		min_x = Plot.center[0] - Plot.sim_params.area_side * \
			Plot.scale - Plot.sim_params.range
		max_x = Plot.center[0] + Plot.sim_params.area_side * \
			Plot.scale + Plot.sim_params.range
		min_y = Plot.center[1] - Plot.sim_params.area_side * \
			Plot.scale - Plot.sim_params.range
		max_y = Plot.center[1] + Plot.sim_params.area_side * \
			Plot.scale + Plot.sim_params.range

		Plot.ax1.add_patch(plt.Rectangle(
			(0, 0), Plot.sim_params.area_side, Plot.sim_params.area_side, fill=None, alpha=1))
	...
	def plot_legend(self, min_x, min_y):
		...
		legend_status = plt.legend(
			handles=patches, loc=3, handlelength=0, handletextpad=0)
